(function () {
            ...
            for (var i = 0; i < attrs.length; i++) {
                ...
                if (attrName.indexOf("nw-") === 0) {
                    ...
                    if (!el.__nw_is_repeat && !el.__nw_is_template) {
                        var attrValue =
                            typeof attrs[i].value !== "undefined"
                            ? attrs[i].value
                            : attrs[i].nodeValue;
        ...
        invalidate: function (bindings, indent, selfCall) {
            var createBindingsTree = function (b) {
                ...
                for (var ii = 0; ii < b.length; ii++) {
                    if (!b[ii]) continue;
                    newObj[ii] = {};
                    newObj[ii].expr = b[ii].expr;
                    newObj[ii].val = b[ii].getValue();

                    if (!!b[ii].subBindings)
                        newObj[ii].sub = createBindingsTree(b[ii].subBindings);
                }
                return newObj;
            };
            ...
            var getTotalBindingsCount = function (b) {
                ...
                for (var ii = 0; ii < b.length; ii++) {
                    if (!b[ii]) continue;
                    if (!!b[ii].subBindings)
                        declCount += getTotalBindingsCount(b[ii].subBindings);
            ...
            if (typeof bindings === 'undefined' || bindings === null) {
                nweb.utils.console.error("nweb.bindings is undefined or null !");
                return 0;
            }
    ...
    nweb.utils = {
        ...
        toTypedObject: function (obj) {
            if (obj != null && !!obj.$type) {
                ...
                var newObj = new window[typename](hasTypenameCtor ? nweb.utils.firstPropertyOrUndefined(window[typenameCtor]) : '');
        ...
        normalizeObjectForServer: function (obj) {
            ...
            if (meta !== null && typeof meta !== 'undefined') {
                for (var ii = 0; ii < meta.properties.length; ii++) {
                    var property = meta.properties[ii];
                    if (property.attrs.indexOf("ClientOnly") !== -1)
                        excludedFields.push("get_" + property.name);
                }

                for (var iii = 0; iii < meta.fields.length; iii++) {
                    var field = meta.fields[iii];
    ...
    window.System_Environment = {};
    window.System_Environment.get_NewLine = function () {
        return "\n";
    };
